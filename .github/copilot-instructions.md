...
- Mutating solver variants (e.g. `lemke_howson!`) follow the QuantEcon.jl `lcp_lemke!` pattern: caller-owned output and primary workspace arrays are positional, auxiliary workspace arrays are keyword arguments defaulting to `nothing` and materialized lazily (keyword defaults are evaluated at call time even if the function returns early)
- Assert non-aliasing with `Base.mightalias` among caller-supplied arrays that the algorithm overwrites before reading others (e.g. `argmins` vs `bases` in `lemke_howson!`), and between the members of same-shaped array pairs
